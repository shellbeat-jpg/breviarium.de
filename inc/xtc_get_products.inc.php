...
function xtc_get_products($session) {
  ...
  //EOF - Dokuman - 2009-11-30 - check for array in cart
      while (list($products_id, ) = each($session['cart']->contents)) {
        $products_query = xtc_db_query("select p.products_id, p.Title as products_name,p.products_image, p.products_model, p.products_price, p.products_discount_allowed, p.products_weight, p.products_tax_class_id from " . TABLE_PRODUCTS . " p where p.products_id='" . xtc_get_prid($products_id) . "'");
