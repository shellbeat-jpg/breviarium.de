...
/* -----------------------------------------------------------------------------------------
  ...
  if((strpos(strtolower($_SERVER['HTTP_USER_AGENT']), 'bot')===false) && (strpos(strtolower($_SERVER['HTTP_USER_AGENT']), 'wget')===false) && (strpos(strtolower($_SERVER['HTTP_USER_AGENT']), 'crawler')===false) && (strpos(strtolower($_SERVER['HTTP_USER_AGENT']), 'spider')===false)){
    ...
    if($wo_ip_address=='64.113.32.29')
			exit;

    if($wo_ip_address=='77.12.128.142') {
			 #global $_GET;
       #echo xtc_get_all_get_params();
			 #echo trim(strip_tags($_GET['list']));
   ...
   //EOF - Dokuman - 2009-10-28 - Who is online doesn't show any IP addresses and URLs (added http_referer)
    ...
    $wo_user_agent = xtc_db_input($_SERVER['HTTP_USER_AGENT']);
    ...
    //xtc_db_query("insert into " . TABLE_WHOS_ONLINE . " (customer_id, full_name, session_id, ip_address, time_entry, time_last_click, last_page_url) values ('" . $wo_customer_id . "', '" . $wo_full_name . "', '" . $wo_session_id . "', '" . $wo_ip_address . "', '" . $current_time . "', '" . $current_time . "', '" . $wo_last_page_url . "')");
      xtc_db_query("insert into " . TABLE_WHOS_ONLINE . " (user_agent, customer_id, full_name, session_id, ip_address, time_entry, time_last_click, last_page_url, http_referer) values ('" . $wo_user_agent . "','" . $wo_customer_id . "', '" . $wo_full_name . "', '" . $wo_session_id . "', '" . $wo_ip_address . "', '" . $current_time . "', '" . $current_time . "', '" . $wo_last_page_url . "', '" . $wo_referer . "')");
    //BOF - Dokuman - 2009-10-28 - Who is online: added http_referer
    }
    }
  }
