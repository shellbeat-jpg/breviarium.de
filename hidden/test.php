...
$url = "http://buch.breviarium.de/hidden/test.php?&chapter=$chapter&STEP=$STEP&k=$k&c=$c&limit=$limit&page=$page&MinimumPrice=$MinimumPrice";
...
# Scriptabgleich URL
if($_SERVER['REQUEST_URI']== "/hidden/test.php"){
	...
	if($amazonCheck!='http://buch.breviarium.de/hidden/test.php'){
	  #header("Location: $amazonCheck");
	  #exit;
	}
}else{
  #header("refresh:70,url=http://buch.breviarium.de/hidden/test.php?&chapter=$chapter&STEP=$STEP&k=$key&c=$c&limit=$limit&page=".($page+1)."&MinimumPrice=$MinimumPrice");
}
$SQL = "update configuration set configuration_value  = 'http://buch.breviarium.de/hidden/test.php?&k=$key&c=$c&limit=$limit&page=".($page+1)."&MinimumPrice=$MinimumPrice' where configuration_key = 'AMAZON_LAST_URL'";
...
if(!isset($arrSteps[$categories_id][$chapter])){
  header("Location: http://buch.breviarium.de/hidden/temp_2nd.php?&chapter=".MIN."&STEP=10&k=$key&c=$c&limit=".($limit+1)."&page=1&MinimumPrice=".MIN);
	exit;
}
...
if($limit>=count($arrCategories)){
	$SQL = "update configuration set configuration_value = 'http://buch.breviarium.de/hidden/test.php' where configuration_key = 'AMAZON_LAST_URL'";
	#mysql_query($SQL) or writeFile("\n$SQL\n".mysql_error()."\n");
...
$c++;
#writeFile("\nnextLocation: http://buch.breviarium.de/hidden/test.php?&k=$key&c=$c&limit=$limit&page=".($page+1)."&MinimumPrice=$MinimumPrice");
#sleep(5);
header("Cache-Control: no-cache, must-revalidate"); // HTTP/1.1
header("Expires: Sat, 26 Jul 1997 05:00:00 GMT"); // Datum in der Vergangenheit
#echo "<br><a href='http://buch.breviarium.de/hidden/test.php?&chapter=$chapter&STEP=$STEP&k=$key&c=$c&limit=$limit&page=".($page+1)."&MinimumPrice=$MinimumPrice'>chapter=$chapter&STEP=$STEP&MinimumPrice=$MinimumPrice&page=".($page+1)."&limit=$limit</a>";
header("Location:http://buch.breviarium.de/hidden/test.php?&chapter=$chapter&STEP=$STEP&k=$key&c=$c&limit=$limit&page=".($page+1)."&MinimumPrice=$MinimumPrice");
exit;
?>