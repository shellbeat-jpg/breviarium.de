...
if($limit>=count($arrCategories)){
 #writeFile("\n$limit>=".count($arrCategories));
  #writeFile("4 ");
	$SQL = "update configuration set configuration_value = 'http://buch.breviarium.de/hidden/temp11.php' where configuration_key = 'AMAZON_LAST_URL11'";
...
#writeFile("5 ");
# Scriptabgleich URL
if($_SERVER['REQUEST_URI']== "/hidden/temp11.php"){
  #writeFile("6 ");
	$check = mysql_query("SELECT configuration_value  FROM configuration where configuration_key = 'AMAZON_LAST_URL11'");
	$amazonCheck = mysql_fetch_array($check, MYSQL_ASSOC);
	$amazonCheck = $amazonCheck['configuration_value'];
	if($amazonCheck!='http://buch.breviarium.de/hidden/temp11.php'){
 ...
 #writeFile("7 ");
 header("refresh:90,url=http://buch.breviarium.de/hidden/temp11.php?&k=$key&c=$c&limit=$limit&page=".($page+1)."&MinimumPrice=$MinimumPrice");
}
#writeFile("8 ");
$SQL = "update configuration set configuration_value  = 'http://buch.breviarium.de/hidden/temp11.php?&k=$key&c=$c&limit=$limit&page=".($page+1)."&MinimumPrice=$MinimumPrice' where configuration_key = 'AMAZON_LAST_URL11'";
mysql_query($SQL) or writeFile("\n".mysql_error()."\n");
...
$c++;
#writeFile("\nnextLocation: http://buch.breviarium.de/hidden/temp11.php?&k=$key&c=$c&limit=$limit&page=".($page+1)."&MinimumPrice=$MinimumPrice");
#sleep(5);
header("Cache-Control: no-cache, must-revalidate"); // HTTP/1.1
header("Expires: Sat, 26 Jul 1997 05:00:00 GMT"); // Datum in der Vergangenheit
header("refresh:0, url=http://buch.breviarium.de/hidden/temp11.php?&k=$key&c=$c&limit=$limit&page=".($page+1)."&MinimumPrice=$MinimumPrice");
exit;
?>