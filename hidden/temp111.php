...
if($limit>=count($arrCategories)){
 #writeFile("\n$limit>=".count($arrCategories));
  #writeFile("4 ");
	$SQL = "update configuration set configuration_value = 'http://buch.breviarium.de/hidden/temp111.php' where configuration_key = 'AMAZON_LAST_URL111'";
...
#writeFile("5 ");
# Scriptabgleich URL
if($_SERVER['REQUEST_URI']== "/hidden/temp111.php"){
  #writeFile("6 ");
	...
	if($amazonCheck!='http://buch.breviarium.de/hidden/temp111.php'){
 ...
 #writeFile("7 ");
 header("refresh:90,url=http://buch.breviarium.de/hidden/temp111.php?&k=$key&c=$c&limit=$limit&page=".($page+1)."&MinimumPrice=$MinimumPrice");
}
#writeFile("8 ");
$SQL = "update configuration set configuration_value  = 'http://buch.breviarium.de/hidden/temp111.php?&k=$key&c=$c&limit=$limit&page=".($page+1)."&MinimumPrice=$MinimumPrice' where configuration_key = 'AMAZON_LAST_URL111'";
mysql_query($SQL) or writeFile("\n".mysql_error()."\n");
...
header("refresh:0, url=http://buch.breviarium.de/hidden/temp111.php?&k=$key&c=$c&limit=$limit&page=".($page+1)."&MinimumPrice=$MinimumPrice");
exit;
?>