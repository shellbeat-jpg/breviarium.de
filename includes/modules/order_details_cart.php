...
for ($i = 0, $n = sizeof($products); $i < $n; $i ++) {
  ...
  //BOF - Dokuman - 15.08.2009 - show 'delete button' in shopping cart
	$del_button = '<a href="'
				  . xtc_href_link(FILENAME_SHOPPING_CART, 'action=remove_product&prd_id=' . $products[$i]['id'] . '&CartItemId='.$products[$i]['CartItemId'], 'SSL')
  ...
  //EOF - Dokuman - 15.08.2009 - show 'delete button' in shopping cart
	if($products[$i]['price']<0.01){
	  $mark_stock  = "*";
	  $_SESSION['any_out_of_stock'] = 1;
	}
	
	$module_content[$i] = array ('PRODUCTS_NAME' => $products[$i]['name'].$mark_stock,
	                              'ConditionNote' => $product->offers[$i]['ConditionNote'],
	                              'SubCondition' => $product->offers[$i]['SubCondition'],
                               'PRODUCTS_QTY' => xtc_draw_hidden_field('cart_quantity[]', 1)
                                                .xtc_draw_hidden_field('products_id[]', $products[$i]['id'])
                                                .xtc_draw_hidden_field('CartItemId[]', $products[$i]['CartItemId'])
