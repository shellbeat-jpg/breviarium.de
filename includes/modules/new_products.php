...
if (MAX_DISPLAY_NEW_PRODUCTS_DAYS != '0') {
   $date_new_products = date("Y.m.d", mktime(1, 1, 1, date("m"), date("d") - (MAX_DISPLAY_NEW_PRODUCTS_DAYS+2), date("Y")));
 ...
 $new_products_query = xtc_db_query($new_products_query);
 $num_rows = xtc_db_num_rows($new_products_query);
  if ($num_rows > 0) {
    $random_row = xtc_rand(0, ($num_rows - MAX_RANDOM_SELECT_NEW));
    xtc_db_data_seek($new_products_query, $random_row);
...
#$new_products_query = xtc_db_query($new_products_query);
while ($new_products = xtc_db_fetch_array($new_products_query, false)) {
   if ($num_rows < 1 || $i < 10)
...
if (sizeof($module_content) >= 1) {
  ...
  if (!CacheCheck()) {
		...
		$cache_id = CURRENT_TEMPLATE.$new_products_category_id.'-newProducts';
		if ((!isset ($new_products_category_id)) || ($new_products_category_id == '0')) {
