...
if ($listing_split->number_of_rows > 0) {
	...
	$navigation .= '
		<table border="0" width="100%" cellspacing="0" cellpadding="2">
		  <tr>
			<td class="smallText" align="left">'.$listing_split->display_count(TEXT_DISPLAY_NUMBER_OF_PRODUCTS).'</td>
     	<td colspan="1" class="smallText" align="right">'.TEXT_RESULT_PAGE.' '.$listing_split->display_links(MAX_DISPLAY_PAGE_LINKS, xtc_get_all_get_params(array ('page', 'info', 'x', 'y'))," ").'</td>
			</tr>
		</table>';
...
// EOF - Tomcraft - 2009-10-30 - noimage.gif is displayed, when no image is defined
  ...
  $module_smarty->assign('HEADING_TITLE', $HEADING_TITLE);
	...
	while ($listing = xtc_db_fetch_array($listing_query, false)) {
		$rows ++;
...
if ($result != false) {
	...
	$module_smarty->assign('AUTHORS_DROPDOWN', $author_dropdown);
	...
	if (!CacheCheck()) {
		...
		$cache_id = $current_category_id.'_'. $_GET['author'].'_'.$_GET['list'].'_'.$_GET['illustrator'].'_'.$_GET['foreword'].'_'.$_GET['photographer'].'_'.$_GET['filter_id'].'_'.$_GET['page'].'_'.$_GET['keywords'].'_'.$_GET['categories_id'].'_'.$_GET['pfrom'].'_'.$_GET['pto'].'_'.$_GET['x'].'_'.$_GET['y'].'_'.$_GET['sorting_id'];
