...
// ---------------------------------------------------------------------------------------
	function cleanKeyWords($KeyWords) {
		global $metaStopWords, $metaMinLength;
	...
	case FILENAME_PRODUCT_INFO : 
		...
		if($product->isProduct()) { 
			...
			if(!empty($product->data['products_meta_keywords'])) { 
				...
				$meta_keyw = metaKeyWords($product->data['Author'].' '.$product->data['products_name'].' - '.$product->data['ISBN'] .' '.$product->data['Publisher'] .' '.$product->data['Binding']);
			}
			...
			if(!empty($product->data['products_meta_description'])) { 
				...
				$meta_descr = $product->data['Author'].': '.$product->data['products_name'].' - '.$product->data['ISBN'] .' '.$product->data['Publisher'] .' '.$product->data['Binding'];
			}
			...
			if(!empty($product->data['products_meta_title'])) {
				...
				$meta_title = metaTitle($product->data['Author'],$product->data['products_name'].' - '.$product->data['ISBN'] .' '.$product->data['Publisher'] .' '.$product->data['Binding'],($addProdShopTitle)?ML_TITLE:'');
	...
	case FILENAME_DEFAULT : 
		...
		if(!empty($current_category_id)) {
			$categories_meta_query = xtDBquery("
				select 	*	from 	".TABLE_CATEGORIES_DESCRIPTION."
			...
			$CATEGORIES_NAME = $categories_meta['categories_name'];
			$CATEGORIES_HEADING_TITLE = $categories_meta['categories_heading_title'];


			
			isset($_GET['page'])?$s=$_GET['page']:$s=1;
	    $listing_sql = xtDBquery("select p.*  from   ".TABLE_PRODUCTS_TO_CATEGORIES." p2c, ".TABLE_PRODUCTS." p
	                                  where
	                                   p.products_id = p2c.products_id
	                                   and  p.products_status = '1'
	                                  and p2c.categories_id = '".$current_category_id . "'
																		order by p.products_last_modified desc
																		Limit " . (($s-1)* MAX_DISPLAY_SEARCH_RESULTS) . ", " . (($s)* MAX_DISPLAY_SEARCH_RESULTS)) ;
			while ($listing = xtc_db_fetch_array($listing_sql, true)) {
	      $tempDescription.= $listing['Author']. " ";
				#$tempKeywords.= $listing['Author']. " ";
				$tempKeywords.= $listing['Title']. " ";
				$tempKeywords.= $listing['Publisher']. " ";
				#$tempKeywords.= $listing['ISBN']. " ";
				#$tempKeywords.= $listing['Binding']. " ";
			}
			$tempKeywords = str_replace(array('&nbsp;','\t','\r','\n','\b','(',')','.','-',',',':', '?', '[', ']'), ' ', $tempKeywords); // <-- Besser bei Zeilenumbrüchen
      $tempDescription = str_replace(array('&nbsp;','\t','\r','\n','\b','(',')','.','-',',',':', '?', '[', ']' ), ' ', $tempDescription); // <-- Besser bei Zeilenumbrüchen
			$arrTempDescription   = explode(' ', $tempDescription);
			$arrTempKeywords   = explode(' ', $tempKeywords);
			for($i=0; $i < count($arrTempKeywords); $i++){
					$first = substr($arrTempKeywords[$i],0 , 1);
					if(strlen($arrTempKeywords[$i])>3 && strtoupper($first) == $first){
					}else{
            $arrTempKeywords[$i]="";
					}
			}
			for($i=0; $i < count($arrTempDescription); $i++){
					$first = substr($arrTempDescription[$i],0 , 1);
					if(strlen($arrTempDescription[$i])>3 && strtoupper($first) == $first){
					}else{
            $arrTempDescription[$i]="";
					}
			}
			$arrTempDescription = array_unique($arrTempDescription);
			$arrTempKeywords = array_unique($arrTempKeywords);
			$strTempDescription = join($arrTempDescription, ' ');
			$strTempKeywords = join($arrTempKeywords, ' ');
			#$strTempDescription = str_replace(array('&nbsp;','\t','\r','\n','\b'),' ',preg_replace("/<[^>]*>/",' ',$strTempDescription)); // <-- Besser bei Zeilenumbrüchen
      #$strTempKeywords = str_replace(array('&nbsp;','\t','\r','\n','\b'),' ',preg_replace("/<[^>]*>/",' ',$strTempKeywords)); // <-- Besser bei Zeilenumbrüchen
			$strTempDescription    = str_replace("\"", "'", $strTempDescription);
			$strTempKeywords    = str_replace("\"", "'", $strTempKeywords);
		...
		if(!empty($categories_meta['categories_meta_title'])) { 
		 ...
		 $meta_keyw =  $strTempKeywords;
		 if(!empty($current_category_id))
         $meta_descr .= " von: " ;
     $meta_descr .=   $strTempDescription;
...
#$meta_title= utf8_decode($meta_title);
				
$xmtitle =  explode(" - ",metaClean(utf8_decode($meta_title)));
#$xmtitle[0] = utf8_decode($xmtitle[0]);
...
<meta name="verification" content="4381d7e295e1a5814d4d2ff003074b14" />
