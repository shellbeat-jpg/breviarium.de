...
if (DISPLAY_PAGE_PARSE_TIME == 'true' && CURRENT_TEMPLATE != 'mobile') {
	$time_start = explode(' ', PAGE_PARSE_START_TIME);
	$time_end = explode(' ', microtime());
	$parse_time = number_format(($time_end[1] + $time_end[0] - ($time_start[1] + $time_start[0])), 3);
...
if (!isset($_SESSION['isBot']) && !isset ($_SESSION['customer_id']) && (strpos(strtolower($_SERVER['HTTP_USER_AGENT']), 'wget')===false) && (strpos(strtolower($_SERVER['HTTP_USER_AGENT']), 'crawler')===false) && (strpos(strtolower($_SERVER['HTTP_USER_AGENT']), 'spider')===false)) {
?>
<script type="text/javascript">

  var _gaq = _gaq || [];
  _gaq.push(['_setAccount', 'UA-3588031-5']);
...
# Weiterleitung Suchmaschinen => Suchanfrage
