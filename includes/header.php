...
<?php include('web4design-de/bestsellercharts/vars/bestsellerinclude.php'); ?>
<?php
	...
	if (node.nodeType == 3) { // text node
		...
		if (tempNodeVal.indexOf(tempWordVal) != -1) {
			...
			if (pn.className != "searchword"+w && pn.className !=  "header") {
				...
				docWordStart = nv.substr(ni-1,1);
				docWordEnd = nv.substr(ni+word.length,1);
				if(((docWordStart==" ") || docWordStart=="." || docWordStart=="," || docWordStart==":" ) && ((docWordEnd==" ") || docWordEnd=="." || docWordEnd=="," || docWordEnd==":" )){
...
function check_form() {
  ...
  var pfrom_float;
  var pto_float;
  var date_added = document.getElementById("advanced_search").date_added.value;
  if (date_added == '' &&  (keywords == '' || keywords.length < 1) && (pfrom == '' || pfrom.length < 1) && (pto == '' || pto.length < 1) ) {
...
echo '<body>';
if($strRestyle!="")
   echo "<div align='center'><b>Ihre Suchbegriffe sind farblich unterlegt: $strRestyle</b></div>";
}
