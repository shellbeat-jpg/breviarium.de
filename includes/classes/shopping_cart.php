...
// EOF - Tomcraft - 2009-11-07 - Added sortorder to products_options
	...
	function getConditionNote($products_id, $MerchantId){
	  global  $arrCondition;
	  #echo "$products_id, $MerchantId";
		$data_reviews = array ();
		$reviews_query = xtDBquery("SELECT SubCondition, ConditionNote
																FROM amazonOffers
																WHERE id_amazon = ".$products_id."
																AND MerchantId = '$MerchantId'");
		if (xtc_db_num_rows($reviews_query, true)) {
		  $reviews = xtc_db_fetch_array($reviews_query, true);
			$data_reviews['SubCondition'] = $arrCondition[$reviews['SubCondition']];
      $data_reviews['ConditionNote'] = $arrCondition[$reviews['ConditionNote']];
		}
		return $data_reviews;
	}
	
	function add_cart($products_id, $qty = '1', $attributes = '', $notify = true, $OfferListingId = '', $ASIN = '') {
		...
		if ($this->in_cart($products_id)) {
		  ...
		  if(isset($_SESSION['CartId']) && $_SESSION['CartId'] !=''){
			  # ShoppingCart anlegen und Titel Hinzufügen
				$params = array("AssociateTag"=>AWS_ASSOC_ID, "Operation"=>"CartAdd", "HMAC"=>$_SESSION['HMAC'], "CartId"=>$_SESSION['CartId'],  "Item.1.OfferListingId"=>$OfferListingId,"Item.1.Quantity"=>"1") ;
				$pxml = aws_signed_request('de', $params, AWS_KEY, AWS_SECRET_KEY);
			}else{
			  # ShoppingCart anlegen und Titel Hinzufügen
				$params = array("AssociateTag"=>AWS_ASSOC_ID, "Operation"=>"CartCreate", "Item.1.OfferListingId"=>$OfferListingId,"Item.1.Quantity"=>"1") ;
				$pxml = aws_signed_request('de', $params, AWS_KEY, AWS_SECRET_KEY);
	      $_SESSION['CartId'] = (string) $pxml->Cart->CartId;
	      $_SESSION['HMAC'] = (string) $pxml->Cart->HMAC;
	      $_SESSION['URLEncodedHMAC'] = (string) $pxml->Cart->URLEncodedHMAC;
	      $_SESSION['PurchaseURL'] = (string) $pxml->Cart->PurchaseURL;
			}
			$CartItems=$pxml->Cart->CartItems;
			for($i=0; $i < count($CartItems); $i++){
			 if($ASIN == (string) $CartItems[$i]->CartItem->ASIN){
	         $this->contents[$products_id]['ASIN']= (string) $ASIN;
					 $this->contents[$products_id]['CartItemId']= (string) $CartItems[$i]->CartItem->CartItemId;
					 $this->contents[$products_id]['MerchantId']= (string) $CartItems[$i]->CartItem->MerchantId;
					 $this->contents[$products_id]['SubAmount']= (string) $CartItems[$i]->SubTotal->Amount;
					 $this->contents[$products_id]['SubCurrencyCode']= (string) $CartItems[$i]->SubTotal->CurrencyCode;
					 $this->contents[$products_id]['SubFormattedPrice']= (string) $CartItems[$i]->SubTotal->FormattedPrice;
					 $this->contents[$products_id]['ConditionNotes']=$this->getConditionNote($products_id, $this->contents[$products_id]['MerchantId']);
				}
			}
...
// EOF - Tomcraft - 2009-11-28 - Included xs:booster
	...
	function remove($products_id, $CartItemId='') {
		//BOF - DokuMan - 2010-03-06 - unset instead of NULL
		//$this->contents[$products_id]= NULL;
		
		# Titel löschen
		$params = array("AssociateTag"=>AWS_ASSOC_ID, "Operation"=>"CartModify", "HMAC"=>$_SESSION['HMAC'], "CartId"=>$_SESSION['CartId'],  "Item.1.CartItemId"=>$CartItemId,"Item.1.Quantity"=>"0") ;
		$pxml = aws_signed_request('de', $params, AWS_KEY, AWS_SECRET_KEY);
	...
	function calculate() {
		...
		while (list ($products_id,) = each($this->contents)) {
			...
			if ($product = xtc_db_fetch_array($product_query)) {
				...
				# echo "$products_id: " . $this->contents[$products_id]['SubAmount'] . "<br>";
			  $products_price =$this->contents[$products_id]['SubAmount']/100 ;
	...
	function get_products() {
		...
		while (list ($products_id,) = each($this->contents)) {
			...
			if ($products = xtc_db_fetch_array($products_query)) {
				...
				#echo 'CartItemId: ' . $this->contents[$products_id]['CartItemId']    ;
				$products_array[] = array (
				'id' => $products_id,
				'name' => $products['products_name'],
				'CartItemId' => $this->contents[$products_id]['CartItemId'],
				'model' => $products['products_model'],
				'image' => $products['products_image'],
				'price' => $this->contents[$products_id]['SubAmount']/100,
				#'price' => $products_price + $this->attributes_price($products_id),
				'quantity' => $this->contents[$products_id]['qty'],
				'SubCondition' => $this->contents[$products_id]['ConditionNotes']['SubCondition'],
				'ConditionNote' => $this->contents[$products_id]['ConditionNotes']['ConditionNote'],
				'weight' => $products['products_weight'],
				'shipping_time' => $main->getShippingStatusName($products['products_shippingtime']),
				#'final_price' => ($products_price + $this->attributes_price($products_id)),
				'final_price' => $this->contents[$products_id]['SubAmount']/100,
