...
if (isset ($_GET['action'])) {
    ...
    //BOF - Dokuman - 15.08.2009 - show 'delete button' in shopping cart
		case 'remove_product':
			...
			$_SESSION['cart'] -> remove($prd_id, $_GET['CartItemId']);
    ...
    //EOF - Dokuman - 15.08.2009 - show 'delete button' in shopping cart
		...
		//BOF - Hetfield - 2009.08.18 - Bugfix for numeric quantitys
			for ($i = 0, $n = sizeof($_POST['products_id']); $i < $n; $i++) {
					...
					if ($cart_quantity < 1 || in_array($_POST['products_id'][$i], (is_array($_POST['cart_delete']) ? $_POST['cart_delete'] : array ()))) {
					$_SESSION['cart']->remove($_POST['products_id'][$i], $_POST['CartItemId'][$i]);
					...
					$_SESSION['cart']->add_cart($_POST['products_id'][$i], $cart_quantity, $attributes, false, $_POST['OfferListingId'], $_POST['ASIN']);
					unset($cart_quantity);
				}
			}
			#xtc_redirect(xtc_href_link($goto, xtc_get_all_get_params($parameters), 'SSL'));
        ...
        $_SESSION['cart']->add_cart((int) $_POST['products_id'], $_SESSION['cart']->get_quantity(xtc_get_uprid($_POST['products_id'], $_POST['id'])) + xtc_remove_non_numeric($_POST['products_qty']), $_POST['id'], true, $_POST['OfferListingId'], $_POST['ASIN']);
		...
		case 'add_a_quickie' :
			...
			if (xtc_has_product_attributes($quickie['products_id'])) {
				...
				if ($_POST['quickie'] != '') {
					...
					$_SESSION['cart']->add_cart($quickie['products_id'], $act_qty +1, 1, true, $_POST['OfferListingId'], $_POST['ASIN']);
			...
			if (isset ($_GET['BUYproducts_id'])) {
				...
				if (xtc_has_product_attributes($_GET['BUYproducts_id'])) {
					...
					if (isset ($_SESSION['cart'])) {
						...
						$_SESSION['cart']->add_cart((int) $_GET['BUYproducts_id'], $_SESSION['cart']->get_quantity((int) $_GET['BUYproducts_id']) + 1, '', true, $_GET['OfferListingId'], $_GET['ASIN']);
					} else {
		...
		case 'cust_order' :
			if (isset ($_SESSION['customer_id']) && isset ($_GET['pid'])) {
				if (xtc_has_product_attributes((int) $_GET['pid'])) {
					...
					$_SESSION['cart']->add_cart((int) $_GET['pid'], $_SESSION['cart']->get_quantity((int) $_GET['pid']) + 1, '', true, $_POST['OfferListingId'], $_POST['ASIN']);
				}
			}
			xtc_redirect(xtc_href_link($goto, xtc_get_all_get_params($parameters), 'SSL'));
			break;
